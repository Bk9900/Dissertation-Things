...
\begin{figure}
	...
	\scalebox{1.5}{\begin{tikzpicture}
		...
		\node at (4,0) (1s) {$1$};
		\node at (4,2) (0s) {$0$};
		\draw [thick,->] (0) -- node [above] {$p$} (0s);
		\draw [thick,->] (1) -- node [below] {$p$} (1s);
		\draw [thick,->] (0) -- node [left = 1em] {$1-p$} (1s);
		\draw [thick,->] (1) -- (0s);
		%\draw [thick, decorate,
		decoration = {brace}] (-0.2,-0.2) --  (-0.2,2.2);
		%\draw [thick, decorate,
		decoration = {brace}] (4.2,2.2) --  (4.2,-0.2);
		\node at (-0.5,1) [left] {$\substack{\text{Transmitted}\\\text{bit}}$};
		\node at (4.5,1) [right] {$\substack{\text{Received}\\\text{bit}}$};
	\end{tikzpicture}}
...
\begin{definition}
	...
	\end{itemize}
\end{definition}
Note the notation of operators written in lower case rather than upper case as is common. This is because we are treating the operators as elements of an algebra, rather than individual operators in their own right.\\
...
\end{theorem}
In other words, $M$ induces a tensor factorisation $\mathcal{H}=\mathcal{H}_{A}\otimes\mathcal{H}_{\ol{A}}$, and it is then the set of all linear operators on the tensor factor $\mathcal{H}_{A}$. For more general von Neumann algebras, this classification generalises.
\begin{theorem}
...
\begin{equation}\label{decomp}
	\mathcal{H}_{L}=\oplus_{\alpha}(\mathcal{H}_{a_{\alpha}}\otimes\mathcal{H}_{\ol{a}_{\alpha}})
\end{equation} 
...
\end{equation}
We can also extend the standard definition of von Neumann entropy to that of \textit{von Neumann entropy on a von Neumann algebra}. Suppose we have a state $\tilde{\rho}$ with support on $\Hcode$, which has diagonal blocks $\tilde{\rho}_{\alpha\alpha}$ with respect to the decomposition \ref{decomp}. We then define
\begin{equation}
