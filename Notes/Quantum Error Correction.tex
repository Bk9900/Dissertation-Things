...
\begin{document}
	...
	\begin{itemize}
		...
		\item 
%		\item For concreteness, say $\dim{\mathcal{H}_{A}}=a$ and $\dim{\mathcal{H}_{\overline{A}}}=\overline{a}$, so $\dim{\mathcal{H}}=a\overline{a}$; also say that $\dim{\mathcal{H}_{\text{code}}}=c\leq a\overline{a}$
%		\item We can represent any state $\rho$ with support in $\mathcal{H}_{\text{code}}$ as an $a\overline{a}\times a\overline{a}$ matrix on $\mathcal{H}$
%		\item The action of the quantum erasure channel (which may erase $\overline{A}$) is then given by
%		$$
%			\mathcal{E}(\rho)=(1-p)\rho+p\,\text{Tr}_{\overline{A}}(\rho)
%		$$
%		\item Define $\{\ket{a}\}$ and $\{\ket{\overline{a}}\}$ as orthonormal bases of $\mathcal{H}_{A}$ and $\mathcal{H}_{\overline{A}}$ respectively; then
%		$$
%			\mathcal{E}(\rho)=(1-p)\rho+p\sum_{\overline{a}}\braket{\overline{a}|\rho|\overline{a}}
%		$$
%		\item Therefore, the operation elements of $\mathcal{E}$ are:
%		$$
%			E_{0}=\sqrt{1-p}I_{A\overline{A}},\quad E_{a}=\sqrt{p}(I_{A}\otimes \bra{\overline{a}})
%		$$
%		\item We can then compute:
%		$$
%		\begin{aligned}
%			E_{0}^{\dagger}E_{0}&=(1-p)I_{A\overline{A}}\\
%			E_{a}^{\dagger}E_{a}&=p\left(I_{A}\otimes\ketbras{\overline{a}}\right)\\
%			E_{0}^{\dagger}E_{a}&=\sqrt{p(1-p)}\left(I_{A}\otimes\bra{\overline{a}}\right)=\sqrt{1-p}E_{a}\\
%			E_{a}^{\dagger}E_{b}&=p\left(I_{A}\otimes\ketbra{\overline{a}}{\overline{b}}\right)
%		\end{aligned}
%		$$
%		\item Since $\{\ket{\tilde{i}}\}$ is an orthonormal basis of $\mathcal{H}_{\text{code}}$, we can write $P_{\text{code}}=\sum_{\tilde{i}}\ketbras{\tilde{i}}$; checking the error correction conditions, we then find:
%		$$
%		\begin{aligned}
%			P_{\text{code}}E_{0}^{\dagger}E_{0}P_{\text{code}}&=(1-p)P_{\text{code}}\\
%			P_{\text{code}}E_{a}^{\dagger}E_{a}P_{\text{code}}&=p
%		\end{aligned}
%		$$
	\end{itemize}
\end{document}
